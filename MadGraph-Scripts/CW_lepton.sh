...
echo "generate e+ e- > ve~ e- w+ / a z h vl vl~, w+ > j j QCD=99 QED=99">>runMG_$N.sh
echo "add process e+ e- > ve e+ w- / a z h vl vl~, w- > j j QCD=99 QED=99">>runMG_$N.sh
echo "output clockwork_"$N"_"$m1"_GeV_"$E"_GeV">>runMG_$N.sh
...
echo "set ebeam1 "$Ehalf".0">>runMG_$N.sh
echo "set ebeam2 "$Ehalf".0">>runMG_$N.sh
echo "set cut_decays True">>runMG_$N.sh
echo "set ptj 20.0">>runMG_$N.sh
echo "set ptl 20.0">>runMG_$N.sh
